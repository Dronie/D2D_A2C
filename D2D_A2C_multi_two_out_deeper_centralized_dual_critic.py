...
beta = 0.01 # socialism parameter (Lower - More Individualist | Higher - More Socialist)
...
combination_params = Socialism_params(name="soc_params", socialism_learning_rate=socialism_learning_rate)

print('Instantiated Socialism Parameters')
...
with tf.Session() as sess:
    ...
    for ep in range(1, train_episodes):
        ...
        _, total_loss_social_critic = sess.run([social_central_critic.combined_optim,
                                        social_central_critic.combined_loss_], feed_dict={
        ...
        _, total_loss_critic, comb_return, advantage = sess.run([combination_params.critic_optim,
                                        combination_params.critic_loss_], feed_dict={
            combination_params.individual_input_: np.expand_dims(individualist_advantage, axis=0),
            combination_params.social_input_: np.expand_dims(socialist_advantage, axis=0),
            combination_params.reward_: np.reshape(socialist_reward, (-1, 1)), # socialist reward is simply the sum of all CUs - could be improved upon w.r.t credit assignment
            combination_params.discount_: np.reshape(discount, (-1, 1)),
            combination_params.bootstrap_: np.reshape(social_bootstrap_values[i], (1,)) 
        })
