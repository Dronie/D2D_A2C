...
# ============================================================================
#
# adjustments made by Stefan Roesch, 2020
#
...
def sequence_advantage_actor_loss(
    advantages, policy_logits, actions, entropy_cost=None, normalise_entropy=False,
    name="SequenceAdvantageActorLoss"):
  ...
  with tf.name_scope(name, values=scoped_values):
    ...
    if entropy_cost is not None:
      ...
      entropy_loss = tf.multiply(
          ...
          tf.reduce_sum(entropy_loss_op, axis=0),
          name="scaled_entropy_loss")  # [B].
      total_loss = tf.add(policy_gradient_loss, entropy_loss,
                          name="total_loss_with_entropy")
    else:
      entropy = None
      entropy_loss = None

    return base_ops.LossOutputNoExtra(total_loss)

#------------------------------------------------------------ Critic_Loss ------------------------------------------------------------

def sequence_advantage_critic_loss(
    baseline_values, rewards,
    pcontinues, bootstrap_value, lambda_=1,
    baseline_cost=1, name="SequenceAdvantageCriticLoss"):
 
  scoped_values = ([baseline_values, rewards, pcontinues, bootstrap_value])
  with tf.name_scope(name, values=scoped_values):
    # Loss for the baseline, summed over the time dimension.
    baseline_loss_td, td_lambda = value_ops.td_lambda(
        baseline_values, rewards, pcontinues, bootstrap_value, lambda_)
    ...
    return base_ops.LossOutputNoExtra(baseline_loss), advantages

#------------------------------------------------------------ Combine_Losses ------------------------------------------------------------

def sequence_advantage_actor_critic_combine(
    policy_logits, baseline_values, actions, rewards,
    pcontinues, bootstrap_value, lambda_=1, entropy_cost=None,
    baseline_cost=1, normalise_entropy=False,
    name="SequenceAdvantageActorCriticLoss"):
