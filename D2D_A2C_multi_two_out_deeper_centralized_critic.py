...
D2D_target_actor_nets = []
D2D_target_critic_nets = []

D2D_target_actor_net_update_ops = []
D2D_target_critic_net_update_ops = []
...
for i in range(0, ch.N_D2D):
    ...
    D2D_critic_nets.append(CriticNetwork(name='c_net_{:.0f}'.format(i),critic_hidden_size=critic_hidden_size,
                                         critic_learning_rate=critic_learning_rate))

    print('Instantiated Critic Network {:.0f} of {:.0f}'.format(i+1, ch.N_D2D))

    D2D_target_critic_nets.append(CriticNetwork(name='c_target_net_{:.0f}'.format(i),critic_hidden_size=critic_hidden_size,
                                                critic_learning_rate=critic_learning_rate))

    print('Instantiated Critic Target Network {:.0f} of {:.0f}'.format(i+1, ch.N_D2D))

    D2D_target_critic_net_update_ops.append(trfl.update_target_variables(D2D_target_critic_nets[i].get_network_variables(), 
                                                                  D2D_critic_nets[i].get_network_variables(), tau=0.001))

    print('Instantiated Critic Target Net Update ops {:.0f} of {:.0f}'.format(i+1, ch.N_D2D))
    print('\n')
...
with tf.Session() as sess:
    ...
    for ep in range(1, train_episodes):
        ...
        if ep == train_episodes:
          ...
          bootstrap_values = []
          for i in range(0, ch.N_D2D):
            bootstrap_values.append(sess.run(D2D_target_critic_nets[i].baseline_, feed_dict={
                D2D_target_critic_nets[i].input_: np.expand_dims(next_state, axis=0)}))
        ...
        for i in range(0, ch.N_D2D):
            ...
            _, total_loss_critic, seq_c_return_, advantage = sess.run([D2D_critic_nets[i].critic_optim,
                                                                       D2D_critic_nets[i].critic_loss_,
                                                                       D2D_critic_nets[i].Critic_return,
                                                                       D2D_critic_nets[i].advantage], feed_dict={
                D2D_critic_nets[i].input_: np.expand_dims(state, axis=0),
                D2D_critic_nets[i].reward_: np.reshape(reward[i], (-1, 1)),
                D2D_critic_nets[i].discount_: np.reshape(discount, (-1, 1)),
                D2D_critic_nets[i].bootstrap_: np.reshape(bootstrap_values[i], (1,)) #np.expand_dims(bootstrap_value, axis=0)
            })
        ...
        for i in range(0, ch.N_D2D):
            sess.run(D2D_target_critic_net_update_ops[i])
